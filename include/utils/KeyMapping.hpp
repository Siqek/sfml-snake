...
{
    constexpr std::array<std::pair<std::string_view, sf::Keyboard::Key>, 101> KeyTable = {{
        { "A",               sf::Keyboard::Key::A },
        { "B",               sf::Keyboard::Key::B },
        { "C",               sf::Keyboard::Key::C },
        { "D",               sf::Keyboard::Key::D },
    ...
    constexpr sf::Keyboard::Key ToKey(std::string_view name)
    {
        for (const auto& [str, key] : KeyTable)
        {
            if (str == name)
            {
                return key;
